...
namespace C3.XNA
{
	...
	public static class Primitives2D
	{
        ...
        #region DrawEndlessLine

        /// <summary>
        /// Draws an endless line that goes through two points
        /// </summary>
        /// <param name="spriteBatch">The destination drawing surface</param>
        /// <param name="point1">The first point</param>
        /// <param name="point2">The second point</param>
        /// <param name="color">The color to use</param>
        /// <param name="thickness">The thickness of the line</param>
        public static void DrawEndlessLine(this SpriteBatch spriteBatch, Vector2 point1, Vector2 point2, Color color, float thickness)
        {
            double dY = 0, dX = 0;                  // Points Y & X distance per X & Y axis
            double a = 0;                           // Line Slope/Gradient
            double b = 0;                           // b - y-intercept
            double X1 = 0, Y1 = 0, X2 = 0, Y2 = 0;  // New End point coordinate bounded by viewport dimentions
            Vector2 P1, P2;                         // New Start/End coordinates of the Line

            // Determine Viewport Limits
            int xViewportWidth = spriteBatch.GraphicsDevice.Viewport.Width;
            int yViewportHeight = spriteBatch.GraphicsDevice.Viewport.Height;

            // if same point, then put pixel.
            if (point1 == point2)
            {
                spriteBatch.PutPixel(point1, color);
                return;
            }

            // Calculate the intersection of the line with the viewport boundaries
            // Y=aX+b
            ...
            {
                // line is parallel to you the y-axis
                X1 = point1.X;
                Y1 = 0;
            ...
            else // a != 0 and a != infinity
            {
                // y=ax+b

                // determine b - y-intercept (when X = 0)
                // b=Y-aX
                b = point1.Y - (a * (double)point1.X);

                // Determine the intersection points with the viewport axis when (X=0, Y=0)
                // Calc Y1 when X=0
                X1 = 0;
                Y1 = b;
